...
public class BaseReader {
	...
	private int GetInventoryData(byte ComAddr,int cmd,byte[]epcdata,int[]epcNum,int[]dlen)
	{
		...
		try{
			...
			}while((System.currentTimeMillis()-beginTime)<maxScanTime+1000*3);
		}catch(Exception e)
		{e.toString();}
		if(callback!=null)
		{
	...
	public int Inventory_G2(byte ComAddr,byte QValue,byte Session,byte AdrTID,byte LenTID,byte Target,
			byte Ant,byte Scantime,byte pOUcharIDList[],int pOUcharTagNum[],int pListLen[])
	{
		byte[]buffer=null;
		if(LenTID==0)
		{
		...
		getCRC(buffer,buffer[0]-1);
		SendCMD(buffer);
		return GetInventoryData(ComAddr,0x01,pOUcharIDList,pOUcharTagNum,pListLen);
	}
	public int Inventory_Temperature(byte ComAddr,
			 byte ENum,
           byte[] EPC,
           byte SelTarget,
           byte SelAction,
           ...
           int pListLen[])
	{
		...
		buffer[4+ENum*2] =(byte)SelTarget;
		buffer[5+ENum*2] =(byte)SelAction;
		buffer[6+ENum*2] =(byte)WaitTime;
		buffer[7+ENum*2] =(byte)QValue;
		buffer[8+ENum*2] =(byte)Session;
		buffer[9+ENum*2] =(byte)Target;
		buffer[10+ENum*2] =(byte)inAnt;
		buffer[11+ENum*2] =(byte)Scantime;
		getCRC(buffer,buffer[0]-1);
		SendCMD(buffer);
		pOUcharTagNum[0]=0;
		pListLen[0]=0;
		return GetInventoryData(ComAddr,0x17,pEPCList,pOUcharTagNum,pListLen);
	}
	...
	public int Lock_G2(byte ComAddr, byte ENum, byte[]EPC,byte select,byte setprotect,byte Password[],
			 byte Errorcode[])
	{
	...
	public int LedOn_kx2005x(byte ComAddr, byte ENum, byte EPC[],
						   byte CarrierTime, byte Password[],byte MaskMem,byte MaskAdr[],byte MaskLen,byte[]MaskData,byte MaskFlag)
	{
		if(MaskFlag==0)
		{
			byte[]buffer = new byte[11 + ENum* 2];
			buffer[0] = (byte)(10 + ENum* 2);
			buffer[1] = ComAddr;
			buffer[2] = (byte)0xCC;
			buffer[3]= ENum;
			if(ENum>0 && ENum<32)
			...
			getCRC(buffer,buffer[0]-1);
			SendCMD(buffer);
			int result = GetCMDData(recvBuff,recvLength,0xCC,3000);
			if(result==0)
			{
				return recvBuff[3]&255;
			}
		...
		else
		{
			...
			byte[]buffer = new byte[15 + maskbyte];
			buffer[0] = (byte)(14 + maskbyte);
			buffer[1] = ComAddr;
			buffer[2] = (byte)0xCC;
			buffer[3]= ENum;
			buffer[ENum*2+4]=CarrierTime;
			System.arraycopy(Password, 0, buffer, ENum*2+5, 4);
			buffer[ENum*2+9]=MaskMem;
			buffer[ENum*2+10]=MaskAdr[0];
			buffer[ENum*2+11]=MaskAdr[1];
			buffer[ENum*2+12]=MaskLen;
			System.arraycopy(MaskData, 0, buffer, ENum*2+13, maskbyte);
			getCRC(buffer,buffer[0]-1);
			SendCMD(buffer);
			int result = GetCMDData(recvBuff,recvLength,0xCC,3000);
			if(result==0)
